...
I've included a multi-stage build example in the `multistage` branch of this repo (`git checkout multistage`). **Note that this is purely shown as a quick and dirty example.** There is much I've done in here that's not great for a real Ruby on Rails app. I got a jumpstart on this by reading this [great article on Medium](https://medium.com/@lemuelbarango/ruby-on-rails-smaller-docker-images-bff240931332) by Lemuel Barango, and his example is much cleaner but I'm not building for production, etc.

In the `multistage` branch you'll find `Dockerfile.multi` which will show you how to remove the dev libraries. Note that this still builds for the Ruby development target - if you were truly doing this for production you'd need to setup secrets, a real database, and all that, which is way beyond our learning here. Because I'm building for the `development` target I still have the sqlite database and libraries which add a few vulns above our 54 vulnerability baseline from our slim starting image. Those disappear when you do `production` builds as it's assumed you have a real database somewhere else.
